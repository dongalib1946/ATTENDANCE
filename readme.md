...
4. `setupSpreadsheet` 함수를 한 번 실행합니다.
   - `출퇴근기록`, `학생명부`, `상호대차 시간표` 시트가 없으면 자동으로 만들어집니다.
   - 예전 버전에서 만들어진 `시간표` 시트가 비어 있으면 삭제하고, 값이 있으면 데이터 보호를 위해 숨김 처리합니다.
...
모니터용 `display.html`은 QR 코드와 함께 오늘 요일의 상호대차 담당 학생을 표시합니다. 시간표는 Google Sheet의 `상호대차 시간표` 시트에서 읽으며, 월요일부터 금요일까지 각 요일별 1타임, 2타임, 3타임 담당자를 관리합니다.
...
- 디스플레이에는 오늘 요일의 1타임, 2타임, 3타임 담당자만 표시됩니다.
- 제목 오른쪽 새로고침 버튼으로 시간표를 수동으로 다시 불러올 수 있으며, 이 요청은 캐시를 우회합니다.
- 열린 디스플레이 화면은 1분마다 백그라운드에서 시간표를 다시 확인하고, 날짜가 바뀌면 자동으로 새 요일 시간표를 강제로 다시 불러옵니다.
- `사용여부`가 `N`이면 해당 요일은 비활성 안내로 표시됩니다.
- Apps Script는 시간표를 30초간 캐시합니다. 즉시 반영해야 하면 새로고침 버튼을 누르거나 Apps Script 편집기에서 `clearScheduleCache` 함수를 한 번 실행하세요.

## 최근 출퇴근 표시

모니터용 `display.html`은 QR 코드와 상호대차 담당자 위에 최근 처리된 출근/퇴근 기록 3건을 표시합니다.

- `출퇴근기록` 시트의 마지막 기록부터 최대 3건을 보여줍니다.
- 화면은 2초마다 백그라운드에서 최근 기록을 다시 확인합니다.
- Apps Script는 최근 기록을 1초간 캐시하며, 출근/퇴근 기록이 저장되면 캐시를 즉시 비웁니다.
- 서버 푸시 방식은 아니지만 최근 출퇴근 영역은 예외적으로 짧은 주기로 확인해 보통 0~2초 수준으로 반영됩니다.
- 모니터 화면에서 `음성 켜기`를 한 번 누르면 이후 새 출근/퇴근 기록이 확인될 때 효과음과 `출근`/`퇴근` 음성이 재생됩니다.
- 표시 정보는 이름, 출근/퇴근 구분, 층, 처리 시각입니다.
